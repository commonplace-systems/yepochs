...
> Every valid edit over the supported Yjs data model, authored in either
> endpoint Yepoch, can be deterministically applied to the other endpoint when
> the bridge is given the required endpoint state.
...
5. Given the required endpoint state, every valid edit over the supported Yjs
   data model can cross from either endpoint to the other.
...
25. conflicting receipt reuse being rejected; and
