...
|> Enum.each(fn {class, rows} ->
  ...
  IO.puts("#{String.pad_trailing(to_string(class), 13)} #{String.pad_trailing(to_string(length(rows)), 4)} " <>
...
IO.puts("   #{length(adj_diff)} of #{Enum.count(results, fn {c,_,_,_,_,_} -> c == :adjacent end)}")
Enum.take(adj_diff, 5) |> Enum.each(fn {_, _, _, _, r1, r2} -> IO.puts("     #{inspect(r1)} vs #{inspect(r2)}") end)

IO.puts("\n== sample DIVERGENT pairs ==")
results |> Enum.filter(fn {_, e, _, _, _, _} -> not e end) |> Enum.take(6)
|> Enum.each(fn {c, _, _, _, r1, r2} -> IO.puts("   #{String.pad_trailing(to_string(c),12)} #{inspect(r1)} vs #{inspect(r2)}") end)
