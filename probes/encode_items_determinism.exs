...
IO.puts(
  "decoded #{length(items)} items across clients #{inspect(Enum.map(items, & &1.id.client) |> Enum.uniq())}"
)
...
for {name, permuted} <- perms do
  ...
  IO.puts(
    "   #{String.pad_trailing(name, 18)} bytes == canonical: #{out == base}  (#{byte_size(out)} vs #{byte_size(base)})"
  )
end
