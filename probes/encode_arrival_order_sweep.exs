...
IO.puts(
  "authoring sizes: full-state=#{byte_size(full.(200, 1, 3))}B  true-delta=#{byte_size(delta.(200, 1, 3))}B"
)
...
rows =
  for {i1, l1} <- ranges, {i2, l2} <- ranges do
    ...
    p1 =
      Encoding.encode_update(integrate.([base_u, d1, d2])) ==
    ...
    conv =
      Text.to_string(integrate.([d1, d2, base_u]), "t") ==
...
|> Enum.each(fn {c, rs} ->
  ...
  IO.puts(
    "#{String.pad_trailing(to_string(c), 14)} #{p.(n)}#{p.(e1)}#{p.(n - e1)}       #{p.(e2)}#{p.(n - e2)}       #{p.(e3)}#{p.(n - e3)}       #{ok}/#{n}"
  )
end)
