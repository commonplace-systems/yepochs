...
⛔ **Scope, per ruling 8.1.** Cases **001–005 are text and deletes**; cases **006–010 are maps and
arrays**, generated for that ruling by `conformance/map_array_corpus.mjs` against the same upstream
`yjs` 13.6.32. ⚠️ **There are still no XML vectors**, so no cross-language claim is made for XML —
and the measured reason is in `docs/design/0002`: element children cannot survive the snapshot
replay at all, so such documents can hold no bridge.

| cases | types | generated by |
|---|---|---|
| 001–005 | text, deletes | inherited from `commonplace-merkle-crdt` |
| 006–008 | `Y.Map` — concurrent distinct keys, sequential overwrite, key deletion | `conformance/map_array_corpus.mjs` |
| 009–010 | `Y.Array` — sequential inserts, element deletion | `conformance/map_array_corpus.mjs` |
